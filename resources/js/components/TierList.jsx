...
const TierList = (initialData) => {
  ...
  const [data, setData] = useState(JSON.parse(initialData.initialData));
  ...
  const downloadScreenshot = () => {
    takeScreenshot(document.querySelector('#tiers')).then(screenshot => {
        ...
        <Container id='tiers'>
          <SortableContext items={data.rows}>
            {data.rows.map((row) => {
              ...
              return <Row key={row.id} id={row.id} name={row.name} color={row.color} items={rowItems} disabled={initialData.disabled} />;
            })}
          </SortableContext>
        </Container>
        <SortableContext items={data.tray.items.map(item => data.items[item])}>
          <Tray key="tray" id="tray" items={data.tray.items.map(item => data.items[item])} disabled={initialData.disabled}></Tray>
        </SortableContext>
        <DragOverlay>
          {initialData.disabled?.trim() != 'true' && activeId ? <Item id={activeId} path={data.items[activeId].path} /> : null}
        </DragOverlay>
      </DndContext>
      <div className="row mt-5">
        <button className='btn btn-secondary' onClick={downloadScreenshot}>Сохранить снимок</button>
      </div>
    </div >
  );
};
...
if (document.getElementById('tierlist')) {
  ...
  const data = document.getElementById('tierlist-data').textContent;
  const disabled = document.getElementById('tierlist-disabled')?.textContent;
  Index.render(
